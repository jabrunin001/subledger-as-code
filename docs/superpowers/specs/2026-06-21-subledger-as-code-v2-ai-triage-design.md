...
balanced-but-wrong-account posting (the `inject_break`), producing a symmetric variance (equal and opposite) on
`cash`/`interest_income` and packaging it as auditor evidence. v2 adds AI-assisted variance triage:
given the reconciliation variances a control surfaces, the system automatically classifies the root cause,
points at the likely locus in the posting logic, and explains it in plain English. Reconciliation exceptions
...
It preserves v1's defining strengths: clone-and-run free, CI green and deterministic, no credentials.
...
- Graceful degradation is the core design. One `TriageEngine` interface with two backends: a deterministic
...
- Finding shape: cluster related variances (a symmetric pair, equal and opposite, is one break), classify from a small
  taxonomy with a confidence level, point at the candidate posting-rules locus, then produce a natural-language
  explanation and recommended next step. No auto-fixing of posting logic (risky and overclaiming).
...
stdlib `urllib` against Ollama's local HTTP API (no SDK). The model is configurable via `OLLAMA_MODEL`
(default a small instruct model, e.g. `llama3.1:8b`); host via `OLLAMA_HOST` (default
`http://localhost:11434`).

## Triage data model and taxonomy
...
A `net_variance` near zero is the deterministic signal separating a *balanced misallocation* (`wrong_account`)
...
This is the default and does real work, not a stub.

Clustering (`cluster.py`, pure): greedily group variances whose signed `variance` values sum to ~0
(within 0.005) into one cluster. A symmetric pair that sums to zero (or an n-way net-zero set) is one underlying break.
A variance that can't be paired into a net-zero group stands alone.

Classification (`heuristic.py`):
- A `net_variance` near zero across two or more accounts classifies as `WRONG_ACCOUNT` at `HIGH` confidence.
  Value was reallocated between accounts: the signature of a misrouted leg (the `inject_break` case).
- `net_variance != 0` classifies as `VALUE_IMBALANCE` at `MEDIUM` confidence. Aggregate value changed,
  which could be a dropped or duplicated leg or an amount error. The explanation notes the global
  trial-balance control also flags true global imbalance, and a single-account residual points at that
  account's source.
- Single account, small residual, no rule match classifies as `UNKNOWN` at `LOW` confidence.
  `TIMING`/`SOURCE_DATA` are reserved; the heuristic does not guess them.

Locus: from `posting_rules`, select rows whose `account_id` is in the cluster to populate `candidate_rules`.
Direction derives from the rules plus each account's `normal_balance` (from `dim_account`), not from
the raw variance sign. The sign's meaning flips between debit-normal and credit-normal accounts, so a naive
"over-stated = wrong destination" rule mislabels direction. The correct derivation: among the cluster's
`candidate_rules`, flag the leg whose legitimate target account shows the unmet expectation (its ledger
balance is missing the value the rule should have posted) as the leading suspect, and identify the other
cluster account as where that value appears to have landed. For the `inject_break` case this
deterministically points at the `payment`/`interest` leg whose target `interest_income` is under-recognized
while an offsetting amount sits in `cash`. The explanation presents this as the leading hypothesis with the
candidate rules attached rather than an over-confident claim. Classification and locus are certain; the
precise directional narrative is "most consistent with."

Explanation and next step (templated, deterministic): assembled from the above, plus a concrete next
...
On the real `inject_break` variances the heuristic alone yields a correct, specific, actionable finding
(the exact mis-routed leg), with no LLM involved.

## Ollama local-LLM backend

Produces the same `TriageFinding` schema; only reasoning/prose quality differs.

- Availability probe: short-timeout request to `OLLAMA_HOST`; if it doesn't answer quickly, the engine
  reports unavailable and falls back. No hang on a fresh clone.
...
- Graceful failure at every step: daemon down, model not pulled, HTTP error, timeout, invalid JSON, or
  Pydantic validation failure all cause that cluster's finding to come from the heuristic with
  `backend="heuristic"`. `--backend ollama` is always at least as good as heuristic.
...
## CLI integration, evidence pack, and CI
...
- With Ollama unavailable (fresh clone / CI), every triage path still produces findings via heuristic
  fallback, with no hard failure and no hang.
