...
controls and a Python CLI that packages auditor-ready evidence. Built as a **portfolio showpiece**
as a portfolio-quality reference implementation of a SOX-style subledger data platform for analytics-engineering roles.

The bar this is optimized against: a recruiter/eng lead skims it in ~5 minutes and a hiring manager
reads deeper. So it must (a) actually run for anyone who clones it, (b) unmistakably signal
dbt + dimensional modeling + SOX/audit + Snowflake competence, and (c) read cleanly top to bottom.

### Key decisions (from brainstorming)

- **Primary goal:** portfolio showpiece (legible signal > learning depth > breadth).
- **Warehouse:** dbt-core + `dbt-duckdb` is the runnable default so it clones-and-runs free in ~60s.
...
- **Domain:** BNPL loan subledger, scoped tight (see below) — realistic for a consumer-lending fintech but bounded for
  provable correctness.
- **Signature feature:** a **toggleable injected break** so the demo shows controls catching a
  realistic posting bug (RED → fix → GREEN), with the audit pack capturing the failure as evidence.
  Mirrors the author's seat-reservation "reproduce the failure, then fix it" pattern.
...
**The heart is `int_postings`:** every business event fans out into balanced double-entry journal
legs, driven by a **posting-rules seed** (`event_type → debit_account, credit_account, amount_expr`)
applied through a **`post_entry` dbt macro**. One design choice yields three must-have signals — macros,
dimensional modeling, correctness-critical logic — in one auditable place. It is also where the
toggleable break lives.
...
- **Canonical facts / marts:**
  ...
  - `fct_balance_rollforward` — grain = account × period: `beginning_balance, originations,
...
- **Source-to-ledger rollforward reconciliation (substantive):** for each account × period,
...
invisible to internal-consistency checks and only a substantive reconciliation catches it — a far
stronger SOX story than an obviously-unbalanced entry.
...
**Tool:** `subledger-audit` (Typer CLI; Pydantic models for the evidence schema — every evidence
artifact is a typed, validated model). Turns a dbt run into an **auditor-ready evidence pack** — the
...
**Demo:** `audit pack` on a clean build → green attestation. Flip `inject_break: true`, run again → the
pack documents the failed control with the exact reconciliation variance. The "Accounting stops
debugging reconciliation by hand" pitch, shown not told.

## Snowflake-portability layer

- **`profiles.yml`** ships both `duckdb` (default/CI) and `snowflake` targets. Models use portable SQL;
  any dialect divergence isolated behind macros.
...
4. Capability map — a table linking each must-have capability (dbt models/tests/macros/CI/docs, dimensional
   modeling, Snowflake, Python, SOX evidence) to where it lives in the repo.
